(function (window) {
	...
	c3_chart_internal_fn.getYDomain = function (targets, axisId) {
		var $$ = this, config = $$.config,
				yTargets = targets.filter(function (d) {
	...
	c3_chart_internal_fn.getTooltipContent = function (d, defaultTitleFormat, defaultValueFormat, color) {
		...
		var mouseX = this.d3.event.pageX;
		var _date = new Date(d[0].x);
		...
		$( "#tooltipExtreme").text(valueFormat(d[3].value, d[3].ratio, d[3].id, d[3].index));
		$( "#tooltipExceptional").text(valueFormat(d[4].value, d[4].ratio, d[4].id, d[4].index));
				...
				if (!config.data_selection_multiple) {
					$$.main.selectAll('.' + CLASS.shapes + (config.data_selection_grouped ? $$.getTargetSelectorSuffix(d.id) : "")).selectAll('.' + CLASS.shape).each(function (d, i) {
						...
						if (shape.classed(CLASS.SELECTED)) {
							toggle.call($$, false, shape.classed(CLASS.SELECTED, false), d, i);
						}
					});
				}
				shape.classed(CLASS.SELECTED, !isSelected);
	...
	c3_chart_fn.data.getAsTarget = function (targetId) {
		var targets = this.data.targets.filter(function (t) {
